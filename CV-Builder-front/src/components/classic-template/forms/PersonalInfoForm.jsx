...
const PersonalInfoForm = ({ data = {}, update, onPhotoUpload, showPhoto = false }) => {
      ...
      {showPhoto && (
        <div className="mb-6 flex items-center gap-6 p-4 bg-gray-50 rounded-xl border border-gray-100">
          <div className="relative group shrink-0">
            <div className={`w-20 h-20 bg-gray-100 border border-gray-200 flex items-center justify-center overflow-hidden transition-all duration-500 ${currentShape === "rounded-full" ? "rounded-full" : "rounded-xl"}`}>
              {uploading ? (
                ...
                <img src={personalData.photo} alt="P" className="w-full h-full object-cover" />
              ) : (
                <Camera size={24} className="text-gray-300" />
              )}
            </div>
            {personalData.photo && !uploading && (
          ...
          <div className="flex-1 space-y-3">
            ...
            <input type="file" ref={fileInputRef} accept="image/*" onChange={handlePhotoChange} disabled={uploading} className="text-[10px] text-gray-400 file:mr-3 file:py-1 file:px-3 file:rounded-md file:border-0 file:bg-gray-100 file:text-gray-600 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed" />
          </div>
        </div>
      )}
